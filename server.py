...
@app.route('/CC_create_user', methods=['POST'])
def CC_create_user():
    data = request.json
    user_id = data.get('CC_user_id')
    if user_id:
...
@app.route('/CC_create_task/<user_id>', methods=['POST'])
def CC_create_task(user_id):
    data = request.json
    title = data.get('CC_title')
    description = data.get('CC_description')
...
@app.route('/CC_list_tasks/<user_id>', methods=['GET'])
...
@app.route('/CC_update_task/<user_id>/<int:task_index>', methods=['PUT'])
def CC_update_task(user_id, task_index):
    data = request.json
    title = data.get('CC_title')
    description = data.get('CC_description')
    if title and description:
...
@app.route('/CC_delete_task/<user_id>/<int:task_index>', methods=['DELETE'])
